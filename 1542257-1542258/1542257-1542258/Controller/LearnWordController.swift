...
protocol protocolEditWordController {
    func editDone(newVocabulary: Vocabulary)
}

class LearnWordController: UIViewController, protocolEditWordController{
    ...
    @IBOutlet weak var labeVietnamese: UILabel!
    ...
    override func viewDidLoad() {
        ...
        vocabularys = DatabaseManagement.shared.getAllVocabulary_Word(cat_id: 1)
        ...
        //let editButton = UIBarButtonItem(barButtonSystemItem: .edit, target: self, action: #selector(buttonEdit_Clicked))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if language == "vi" {
            self.navigationItem.title = "Từ Vựng"
        }

    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SegueEditWordID" {
            let dest = segue.destination as! EditWordController
    ...
    func buttonEdit_Clicked() {
        performSegue(withIdentifier: "SegueEditWordID", sender: nil)
    }
    ...
    func buttonDelete_Clicked() {
        ...
        let okAction = UIAlertAction(title: titleOk, style: .default) { (UIAlertAction) in
            if DatabaseManagement.shared.deleteVocabulary(id: self.vocabularys[self.selectedIndex].id) {
                //print(self.vocabularys[self.selectedIndex].image!)
                // xoa image
    ...
    func loadData(index: Int) {
        ...
        let typeofWord = DatabaseManagement.shared.getTypeofWordByID(id: vocabularys[selectedIndex].type_id!)
        if typeofWord != nil {
            if language == "vi" {
                labelTypeofWord.text = typeofWord?.name_vn
            } else {
                labelTypeofWord.text = typeofWord?.name_en
            }
        } else {
            labelTypeofWord.text = ""
        }
        
        if vocabularys[index].pronunciation != nil {
            labelPronunciation.text = vocabularys[index].pronunciation!
        } else {
            labelPronunciation.text = ""
        }
        
        labeVietnamese.text = vocabularys[index].name_vn
    ...
    // MARK: *** protocolEditWordController 
    func editDone(newVocabulary: Vocabulary) {
