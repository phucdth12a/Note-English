...
class AddWordController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        selectedTypeofWord = typeofWords[0]
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if language == "vi" {
            ...
            self.navigationItem.title = "Thêm Từ Vựng"
    ...
    func buttonAdd_clicked() {
        ...
        if textFieldEnglish.isEmpty() {
            ...
            let id = DatabaseManagement.shared.addVocabulary(cat_id: 1, type_id: (selectedTypeofWord?.id)!, name_en: textFieldEnglish.text!, name_vn: textFieldVietNamese.text!, pronunciation: textFiledPronunciation.text!, example: textViewExample.text, image: image, status: false, created: dateCreated)
            
            
            if id != nil {
                var titleSuccess = "Success"
                var mesageSuccess = "Add Word Success"
                if language == "vi" {
                    titleSuccess = "Thành công"
                    mesageSuccess = "Thêm từ vựng thành công"
                }
                
                // luu vao bang report
                _ = DatabaseManagement.shared.addOrUpdateReport(total_word_true: 0, total_word: 0, total_add_word: 1, total_learned_word: 0, total_phrase_true: 0, total_phrase: 0, total_add_phrase: 0, total_learned_phrase: 0, total_idioms_true: 0, total_idioms: 0, total_add_idioms: 0, total_learned_idioms: 0, created: dateCreated)
