...
class TestPhraseController: UIViewController {
        ...
        if mode == 1 { // english
            ...
            if textFieldAnswer.text?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == vocabularys[selectedIndex].name_vn.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
                ...
                if selectedIndex == vocabularys.count { // da het cau hoi
                    ...
                    _ = DatabaseManagement.shared.addOrUpdateReport(total_word_true: 0, total_word: 0, total_add_word: 0, total_learned_word: 0, total_phrase_true: answerdCorrect, total_phrase: vocabularys.count, total_add_phrase: 0, total_learned_phrase: 0, total_idioms_true: 0, total_idioms: 0, total_add_idioms: 0, total_learned_idioms: 0, created: dateCreated!)
    ...
    override func viewDidLoad() {
        ...
        vocabularys = DatabaseManagement.shared.getVocabularyfoTest(startDate: stringStartDate, endDate: stringEndDate, status: false, cat_id: 2)
        
        print(vocabularys.count)
        ...
        if vocabularys.count > 0 {
            ...
            if language == "vi" {
                title = "Thông báo"
                message = "Không có cụm từ! Vui lòng thêm"
            }
